...
#import "OCR.h"
@interface DrivingLicenceController ()

@property (nonatomic,retain) UIImageView * frontImgView,*frontBgImgView;
@property (nonatomic,retain) UILabel * resultLab;
@end

@implementation DrivingLicenceController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title=@"DrivingLicenceSacn";
    self.view.backgroundColor=[UIColor whiteColor];
    [self ShowResultUI];
   
    UIBarButtonItem *rightBarItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"customerScan"] style:UIBarButtonItemStylePlain target:self action:@selector(drivingLicenceScan)];
...
- (void)drivingLicenceScan{
    ...
    JZCameraViewController *cameraVC = [[JZCameraViewController alloc] init];
    cameraVC.idInfoMenion = ^(NSDictionary *infoDic) {
        ...
        if (infoDic) {
            ...
            Self.frontImgView.image=Dic[@"JZ_IDIMAGE"];
            [Dic removeObjectForKey:@"JZ_IDIMAGE"];
    ...
    cameraVC.ymIDCardEngine = [OCR initializationWithLanguage:2 andIndex:cardType_JZ ];
    
    if (cameraVC.ymIDCardEngine.initSuccess) {
        cameraVC.exIdCardIndex = cardType_JZ;
